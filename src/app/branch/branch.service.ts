          ...
          for (var i = 0; i < branchList.length; i++) {
            if (branchInfo.id === branchList[i].id) {
              ...
              for (var j = 0; j < branchInfo.branchParameters.length; j++) {
                ...
                if (branchInfo.branchParameters[j].key == 'label.address2' && branchInfo.branchParameters[j].value != null) {
                  addressLine2 = branchInfo.branchParameters[j].value + ", ";
                } else if (branchInfo.branchParameters[j].key == 'label.address2' && branchInfo.branchParameters[j].value == null) {
                  addressLine2 = emptyAddressLine;
                }

                if (branchInfo.branchParameters[j].key == 'label.address3' && branchInfo.branchParameters[j].value != null) {
                  addressLine3 = branchInfo.branchParameters[j].value;
                } else if (branchInfo.branchParameters[j].key == 'label.address3' && branchInfo.branchParameters[j].value == null) {
                  addressLine3 = emptyAddressLine;
                }

              }

              //following code removes any trailing commas              
              branchList[i].address = (addressLine1 + addressLine2 + addressLine3).trim();
              let lastIndexOfComma = (branchList[i].address).lastIndexOf(',');

              if (lastIndexOfComma === branchList[i].address.length - 1) {
                branchList[i].address = branchList[i].address.substr(0, lastIndexOfComma - 1);
              }
            }
          }
        },
